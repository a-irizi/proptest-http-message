  ...
  proptest! {
    ...
    fn absolute_works((absolute_form, repr) in absolute(20, 50.try_into().unwrap(), 0..=20)) {
      ...
      if let Some(user_info) = absolute_form.authority.user_info {
        ...
        assert_eq!(user_info.password.as_deref(), url.password(), "expected to get password {:?} but got {:?}", user_info.password, url.password());
      } else {
      ...
      match (absolute_form.authority.port, url.port()) {
        (None | Some(80), None) => {},
        (Some(port), Some(port2)) => assert_eq!(port, port2, "expected port but {port} parsed {port2}"),
        (port, port2) => panic!("expected port but {port:?} parsed {port2:?}")
      }

      match (&absolute_form.path, url.path()) {
        (Some(path), path2) => assert_eq!(path.normalized, path2, "expected path {path:?} but parsed {path2}"),
        (None, path) => assert!(path.is_empty() || path == "/", "expected path to be empty but parsed {path}")
      }
      ...
      match (absolute_form.fragment, url.fragment()) {
        (None, None) => {},
        (Some(fragment), Some(fragment2)) => assert_eq!(fragment, fragment2, "expected fragment {fragment:?} but parsed {fragment2:?}"),
